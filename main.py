...
async def analyze_data(request: Request):
    """
    Main endpoint that accepts multiple file uploads with a required questions file.
    Robust upgrades:
    - Accepts questions file via multiple field names or filename patterns
    - Validates file type/size and possible text encodings
    - Handles additional file metadata
    - Graceful error handling and detailed logging for debug + prod
    """
    task_id = str(uuid.uuid4())
    logger.info(f"[{task_id}] Starting synchronous task")

    try:
        # --- Flexible form parsing ---
        form = await request.form()
        questions_upload = None
        additional_files = []

        for field_name, value in form.items():
            # Handle UploadFile instances
            if hasattr(value, "filename") and value.filename:
                fname_lower = value.filename.lower()

                # Flexible detection of the questions file
                if (
                    "question" in fname_lower and fname_lower.endswith(".txt")
                ) or field_name.lower() in ("questions_txt", "question.txt"):
                    if questions_upload:
                        logger.warning(f"[{task_id}] Multiple candidate question files detected, using first one: {questions_upload.filename}")
                    else:
                        questions_upload = value
                else:
                    additional_files.append(value)
            else:
                # For non-file fields, ignore or log
                logger.debug(f"[{task_id}] Ignoring non-file field: {field_name}")

        # --- Ensure required file exists ---
        if not questions_upload:
            raise HTTPException(
                status_code=400,
                detail="A questions file (questions_txt / question.txt / *question*.txt) is required."
            )

        # --- Read & decode questions file ---
        try:
            questions_content = await questions_upload.read()
            # Try UTF-8 first, else fall back to latin-1 or replace errors
            try:
                questions_text = questions_content.decode("utf-8")
            except UnicodeDecodeError:
                try:
                    questions_text = questions_content.decode("latin-1")
                except Exception:
                    questions_text = questions_content.decode("utf-8", errors="replace")

            logger.info(f"[{task_id}] Processed questions file '{questions_upload.filename}' ({len(questions_text)} chars)")
        except Exception as e:
            logger.error(f"[{task_id}] Failed to read/decode questions file: {e}")
            raise HTTPException(status_code=400, detail=f"Could not read questions file: {e}")
        ...
        for file in additional_files:
            if file.filename:
                ...
                # Detect text files by extension or MIME
                ext = os.path.splitext(file.filename)[-1].lower()
                try:
                    file_text = content.decode("utf-8")
                    file_contents[file.filename] = file_text
                    is_text = True
                    logger.info(f"[{task_id}] Processed text file: {file.filename}")
                except UnicodeDecodeError:
                    file_contents[file.filename] = f"Binary file: {file.filename} ({len(content)} bytes)"
                    is_text = False
                    logger.info(f"[{task_id}] Processed binary file: {file.filename} ({len(content)} bytes)")
        ...
        # --- Prepare task description & workflow ---
        task_description = questions_text.strip()
        detected_workflow = await detect_workflow_type_llm(task_description, "multi_step_web_scraping")
        logger.info(f"[{task_id}] Detected workflow: {detected_workflow}")
        logger.debug(f"[{task_id}] Task description snippet: {task_description[:200]!r}")
        ...
        logger.info(f"[{task_id}] Workflow input ready (keys: {list(workflow_input.keys())})")

        # Execute workflow with timeout & handle fallback
        try:
            result = await asyncio.wait_for(
        ...
        logger.exception(f"[{task_id}] Unhandled error")
        raise HTTPException(status_code=500, detail=f"Internal server error: {e}")


# Keep execute_workflow_sync exactly as in your code but defined above usage
async def execute_workflow_sync(workflow_type: str, workflow_input: Dict[str, Any], task_id: str) -> Dict[str, Any]:
    """Execute workflow synchronously with enhanced error handling"""
    try:
        if orchestrator is None:
            logger.warning(f"[{task_id}] No orchestrator available")
            return {
                "workflow_type": workflow_type,
                "status": "completed_fallback",
        ...
        if workflow_type not in orchestrator.workflows:
            logger.warning(f"[{task_id}] Workflow '{workflow_type}' not found")
            return {
                "workflow_type": workflow_type,
                "status": "error",
                "message": f"Workflow {workflow_type} not found",
        ...
        logger.exception(f"[{task_id}] Error executing workflow {workflow_type}")
        raise
